...
// Real backend adapters (replace the mock defaults). Type-only back-import in
// the adapters file avoids a runtime cycle.
import {
  ApiPackageCatalogBackendAdapter,
  ApiPackageCampaignBackendAdapter,
} from './packageBackendAdapters';

const apiCatalogAdapter = new ApiPackageCatalogBackendAdapter();
const apiCampaignAdapter = new ApiPackageCampaignBackendAdapter();

export const packageCatalogRepository = new CachedPackageCatalogRepository(apiCatalogAdapter);
export const packageCampaignRepository = new CachedPackageCampaignRepository(apiCampaignAdapter);
