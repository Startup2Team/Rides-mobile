...
export default function DriverDashboard() {
  ...
  const { entitlement, isLoading: isEntitlementLoading } =
  ...
  const [licenseBlockVehicle, setLicenseBlockVehicle] = useState<Pick<
    DriverVehicleProfile,
    "id" | "vehicleType" | "plateNumber"
  > | null>(null);
  ...
  const activeVehicleEntitlement = getVehicleEntitlement(
    entitlement,
    activeVehicle,
  );
  ...
  const handleVehicleSessionStart = useCallback(
    async (vehicle: DriverVehicleProfile) => {
      const vehicleEntitlementForSelection = getVehicleEntitlement(
        entitlement,
        vehicle,
      );
      ...
      const vehicleLicenseStatus = getLicenseComplianceStatus(
  ...
  const cycleMapType = () => {
    setMapType(
      (prev) => MAP_TYPES[(MAP_TYPES.indexOf(prev) + 1) % MAP_TYPES.length],
    );
  };
  ...
  const handleAdCarouselMomentumEnd = (
    ...
    const pageIndex = Math.round(
      event.nativeEvent.contentOffset.x / adCarouselWidth,
    );
  ...
  const activeVehicleType =
    activeVehicle?.vehicleType ?? driverProfile?.vehicleType ?? "moto";
  const activitySummary = getDriverActivitySummary({
    driverId: user?.id,
    driverProfile,
    entitlement,
    rideHistory,
  ...
  const requestDestinationLabel = formatRequestLocation(
    request?.destination,
    "Destination unavailable",
  );
  const requestDistanceToPickup = request
    ? formatDistanceToPickup(driverLocation, request.pickup)
  ...
  return (
    <View style={[styles.root, { backgroundColor: dashboardPageBackground }]}>
      ...
      <View
        style={[
          styles.topBar,
          { paddingTop: Platform.OS === "web" ? 67 : spacing[0] },
        ]}
      >
        <View
          style={[
        ...
        >
          <View style={styles.statusHeader}>
            <View style={styles.statusIdentity} testID="driver-identity-block">
              <View style={styles.greetingRow}>
                <AppText
                  style={[styles.statusGreeting, { color: colors.foreground }]}
                  numberOfLines={1}
                >
                  {driverName}
                </AppText>
                {isVerified && <VerifiedBadge testID="driver-verified-badge" />}
              </View>
            ...
            <View style={styles.ctaRow}>
              <View
                ...
                accessibilityActions={[
                  { name: "activate", label: "Switch to customer mode" },
                ]}
              ...
              >
                ...
                >
                  ...
                  >
                    <AppText style={styles.switchModeAvatarText}>
                      {driverInitial}
                    </AppText>
                    {profileImage ? (
                      <Image
                ...
                >
                  <AppText
                    style={[
                      styles.switchModeQuickActionText,
                      { color: colors.primaryForeground },
                    ]}
                    numberOfLines={1}
                  >
                  ...
                  <View
                    style={[
                      styles.notifDot,
                      {
                        ...
                        borderColor: colors.background, // or dashboardSecondaryFill? The user asked for a black circle, white border matches better. colors.background or dashboardSecondaryFill are both fine. Let's use colors.background since it is the screen background and matches HomeTopHeader.
                      },
                    ]}
                  >
                    <Text
          ...
          <AppText style={[styles.activityTitle, { color: colors.foreground }]}>
            Today's Activity
          </AppText>
          <View style={styles.activityGrid}>
            <View style={styles.activityStat}>
              <AppText
                style={[styles.activityValue, { color: colors.foreground }]}
                numberOfLines={1}
          ...
          {showNoCreditsWarning && (
            <View
              style={[
                styles.noCreditsPanel,
                {
                  backgroundColor: colors.successHex + "12",
                  borderColor: colors.successHex + "35",
                },
              ]}
            >
      ...
      {!request && (
        <View style={[styles.onlineBtnWrap, { bottom: tabBarHeight - 8 }]}>
          <Animated.View style={{ transform: [{ scale: onlineScale }] }}>
            ...
            >
              <View
                style={[styles.onlineBtnDot, { backgroundColor: "#fff" }]}
              />
        ...
        <Animated.View
          style={[
            styles.requestSheet,
            {
              ...
              paddingBottom: tabBarHeight + spacing[10],
            },
          ]}
        >
          <View style={styles.requestHeader}>
            ...
            <View style={{ flex: 1, minWidth: 0 }}>
              ...
              {request.customerRating != null ? (
                <View style={styles.requestRatingRow}>
                  ...
                  />
                  <AppText
                    style={[styles.requestRatingText, { color: colors.star }]}
                  >
            ...
            <View
              style={[
                ...
                {
                  backgroundColor:
                    countdown <= 5 ? colors.destructive : colors.primary,
                },
              ]}
            >
          ...
          <View style={styles.metaRow}>
            ...
            >
              ...
              <View style={styles.metaInfoText}>
                ...
                <AppText
                  style={[styles.metaInfoValue, { color: colors.foreground }]}
                >
                  {requestDistanceToPickup}
                </AppText>
              </View>
            </View>
            <View
          ...
          <View style={[styles.selectorSheet, { backgroundColor: cardFill }]}>
            <View style={styles.selectorHeader}>
              <View style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  Select vehicle for this session
                </AppText>
                <AppText
                  style={[
                ...
                >
                  Choose the approved vehicle you are driving right now.
                </AppText>
              </View>
              <TouchableOpacity
                onPress={() => setVehicleSelectorVisible(false)}
            ...
            <ScrollView
              ...
              contentContainerStyle={{ gap: spacing[10] }}
            >
                ...
                return (
                  ...
                  >
                    <View style={styles.selectorCardCopy}>
                      ...
                      >
                        {getRideBalance(vehicleEntitlementForSelection)} rides
                        left
                        {getActiveBonusRides(vehicleEntitlementForSelection) > 0
                          ? ` - ${getActiveBonusRides(vehicleEntitlementForSelection)} bonus rides`
                          : ""}
                      </AppText>
                    </View>
                    <View
                      style={[
                        styles.selectorPill,
                        { backgroundColor: colors.successHex + "14" },
                      ]}
                    >
                      <AppText
                        style={[
      ...
      >
        <View style={styles.selectorBackdrop}>
          ...
          />
          <View
            style={[styles.licenseModalSheet, { backgroundColor: cardFill }]}
          >
            <View style={styles.selectorHeader}>
              <View style={{ flex: 1, minWidth: 0 }}>
                <AppText
                ...
                >
                  {licenseBlockVehicle?.plateNumber
                    ? `${licenseBlockVehicle.plateNumber} is not currently eligible to go online.`
                    : "Your selected vehicle is not currently eligible to go online."}
                </AppText>
              </View>
              <TouchableOpacity
                onPress={closeLicenseBlockModal}
                accessibilityLabel="Close license warning"
              >
                <Feather
                  name="x"
                  size={icons.size.lg}
            ...
            <AppText
              style={[styles.licenseModalBody, { color: colors.foreground }]}
            >
              Your driver license has expired. Update your driver license
              documents to continue receiving ride requests.
            </AppText>
...
const styles = StyleSheet.create({
  ...
  statusDivider: {
    ...
    marginVertical: semanticSpacing.rowGap,
  },
  activityTitle: {
    ...typography.caption,
  ...
  mapBtn: {
    width: sizes.mapControl.md,
    height: sizes.mapControl.md,
    borderRadius: 23,
    alignItems: "center",
    justifyContent: "center",
