...
// Back-compat lookup of package display info keyed by packageId, derived from
// the catalog. Screens use it to show an active/purchased package's name + total.
export const DRIVER_RIDE_PACKAGES: Record<string, {
  id: string;
  name: string;
  normalPriceRwf: number;
  currentPriceRwf: number;
  ...
  DRIVER_RIDE_PACKAGE_CATALOG.map(entry => [entry.packageId, {
    id: entry.packageId,
    name: entry.packageName,
    normalPriceRwf: entry.compareAtPriceRwf ?? entry.priceRwf,
    currentPriceRwf: entry.priceRwf,
    includedRides: entry.ridesGranted,
    bonusRides: entry.bonusRidesGranted,
    totalCredits: entry.ridesGranted + entry.bonusRidesGranted,
    launchOffer: entry.priceRwf === 0,
  }]),
);
