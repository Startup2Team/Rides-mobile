...
//
// Rationale: a WS lifecycle event missed while the socket is down or silently
// stalled (backgrounded app, dead-but-not-yet-detected connection) leaves a
// stale ride on screen. The backend also sends a push for the same events —
// this lets that push self-heal the stale state by triggering the same
// reconcile-against-GET-/rides/active path RideProvider already runs on
// foreground resume and on its backstop interval, instead of doing nothing
// but refresh a badge.
...
/** Called by usePushNotifications when a push implies the ride may have moved on. */
