...
export function RequirementsSection({ acceptedTerms, colors, errors, form, setAcceptedTerms, setErrors, update }: {
  ...
  return <View style={styles.section}>
    ...
    <AppInput label={form.momoProvider === 'mtn' ? 'MTN MoMo Phone Number' : 'Airtel Money Phone Number'} placeholder="250XXXXXXXXX" value={form.momoCode} onChangeText={text => update('momoCode', text.replace(/\D/g, ''))} error={errors.momoCode} leftIcon="smartphone" keyboardType="phone-pad" />
    <AppInput label={form.momoProvider === 'mtn' ? 'MoMo Pay Code' : 'Airtel Merchant Code'} placeholder="e.g. 123456" value={form.merchantCode} onChangeText={text => update('merchantCode', text.toUpperCase())} error={errors.merchantCode} leftIcon="briefcase" autoCapitalize="characters" />
