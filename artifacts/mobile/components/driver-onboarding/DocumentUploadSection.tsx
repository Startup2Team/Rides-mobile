...
export function DocumentUploadSection({ colors, docs, errors, pickDocument, takeDocumentPhoto }: {
  colors: ReturnType<typeof useColors>; docs: Record<DocumentKey, DocFaces>; errors: Record<string, string>;
  ...
  return <View style={styles.section}>
    ...
    {DOCUMENTS.map(document => <View key={document.key} style={styles.docRow}>
      ...
      <DocumentFace colors={colors} uri={docs[document.key][0]} hasError={Boolean(errors[document.key])} onPick={() => pickDocument(document.key, 0)} onTake={() => takeDocumentPhoto(document.key, 0)} />
      ...
      {docs[document.key][0] && <><Text style={[styles.docFaceLabel, { color: colors.mutedForeground, marginTop: 4 }]}>Back face</Text>
        {docs[document.key][1] ? <DocumentFace colors={colors} uri={docs[document.key][1]} hasError={false} onPick={() => pickDocument(document.key, 1)} onTake={() => takeDocumentPhoto(document.key, 1)} />
          : <TouchableOpacity style={[styles.docAddBackBtn, { borderColor: colors.border, backgroundColor: colors.card }]} onPress={() => Alert.alert('Add back face', 'Choose how to add the back face of this document.', [{ text: 'Take photo', onPress: () => takeDocumentPhoto(document.key, 1) }, { text: 'Upload from gallery', onPress: () => pickDocument(document.key, 1) }, { text: 'Cancel', style: 'cancel' }])}><Feather name="plus" size={15} color={colors.mutedForeground} /><Text style={[styles.docAddBackText, { color: colors.mutedForeground }]}>Add another photo (back face)</Text></TouchableOpacity>}
      </>}
    </View>)}
  </View>;
}

function DocumentFace({ colors, hasError, onPick, onTake, uri }: {
  colors: ReturnType<typeof useColors>; hasError: boolean; onPick: () => void; onTake: () => void; uri: string | null;
}) {
  if (uri) return <View style={styles.docPreviewRow}><Image source={{ uri }} style={styles.docThumb} resizeMode="cover" /><View style={{ flex: 1, gap: 6 }}><View style={[styles.docUploaded, { backgroundColor: colors.primaryHex + '15', borderColor: colors.primaryHex + '30' }]}><Feather name="check-circle" size={14} color={colors.primary} /><Text style={[styles.docUploadedText, { color: colors.primary }]}>Uploaded</Text></View><View style={styles.docActionRow}><SmallAction colors={colors} icon="image" label="Change" onPress={onPick} /><SmallAction colors={colors} icon="camera" label="Retake" onPress={onTake} /></View></View></View>;
  return <View style={styles.docActionRow}><UploadAction colors={colors} hasError={hasError} icon="upload" label="Upload" onPress={onPick} /><UploadAction colors={colors} hasError={hasError} icon="camera" label="Camera" onPress={onTake} /></View>;
}
